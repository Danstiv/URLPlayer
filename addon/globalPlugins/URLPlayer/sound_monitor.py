...
class SoundMonitor:
	...
	def loop(self):
		while not self.stop_event.is_set():
			...
			if self.monitor_type==1:
				previous_state = dict(self.active_processes)
				for session in sessions:
					...
					if peak<=self.min_peak and process_info[0] in self.active_processes:
						self.active_processes.pop(process_info[0])
				if previous_state!=self.active_processes:
					self.callback(dict(self.active_processes))
	...
	def update(self, process_info, state):
		temp = len(self.active_processes)
		if state==1 and process_info[0] not in self.active_processes:
			self.active_processes[process_info[0]] = process_info[1]
		if state==0 and process_info[0] in self.active_processes:
			self.active_processes.pop(process_info[0])
		if temp!=len(self.active_processes):
			self.callback(dict(self.active_processes))
